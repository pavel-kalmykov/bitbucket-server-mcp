...
export function registerPrompts(
  ...
  server.registerPrompt(
    'review-pr',
    {
      description: 'Fetch all context needed to review a pull request: details, diff, comments, and CI results.',
      argsSchema: {
    ...
    async ({ project, repository, prId }) => {
      ...
      const [prData, diffData, activitiesData, insightsData] = await Promise.all([
        clients.api.get(basePath).json<Record<string, unknown>>(),
        clients.api.get(`${basePath}/diff`, { searchParams: { contextLines: 10, withComments: false } }).json<Record<string, unknown>>(),
        clients.api.get(`${basePath}/activities`).json<{ values: unknown[] }>(),
        clients.insights.get(`${basePath}/reports`).json<{ values: unknown[] }>().catch(() => ({ values: [] })),
      ]);

      const title = prData.title ?? 'Unknown';
      const author = ((prData.author as Record<string, unknown>)?.user as Record<string, unknown>)?.displayName ?? 'Unknown';
      const description = prData.description ?? 'No description';
      const state = prData.state ?? 'UNKNOWN';

      const comments = (activitiesData.values ?? [])
        .filter((a: unknown) => (a as Record<string, unknown>).action === 'COMMENTED');
