...
export function registerBranchTools(
  ...
  server.registerTool('list_branches', {
    description: 'List branches in a repository. Also returns the default branch when available.',
    inputSchema: {
      project: z.string().optional().describe('Project key. Defaults to BITBUCKET_DEFAULT_PROJECT.'),
      repository: z.string().describe('Repository slug.'),
      filterText: z.string().optional().describe('Filter branches by name substring.'),
      limit: z.number().optional().describe('Number of branches to return (default: 25, max: 1000).'),
    ...
    try {
      ...
      const [branchData, defaultBranch] = await Promise.all([
        clients.api.get(`projects/${resolvedProject}/repos/${repository}/branches`, {
          searchParams,
        }).json<{ values: unknown[]; size: number; isLastPage: boolean }>(),
        clients.api.get(`projects/${resolvedProject}/repos/${repository}/default-branch`)
          .json()
          .catch(() => null),
      ]);
  ...
  server.registerTool('list_commits', {
    description: 'List commits in a repository, optionally filtered by branch and author.',
    inputSchema: {
      project: z.string().optional().describe('Project key. Defaults to BITBUCKET_DEFAULT_PROJECT.'),
      repository: z.string().describe('Repository slug.'),
      branch: z.string().optional().describe('Branch name to list commits from.'),
      author: z.string().optional().describe('Filter commits by author (case-insensitive match on name, slug, or displayName).'),
      limit: z.number().optional().describe('Number of commits to return (default: 25, max: 1000).'),
      start: z.number().optional().describe('Start index for pagination (default: 0).'),
    },
    annotations: { readOnlyHint: true },
    ...
    try {
      ...
      const data = await clients.api.get(`projects/${resolvedProject}/repos/${repository}/commits`, {
        searchParams,
  ...
  server.registerTool('delete_branch', {
    description: 'Delete a branch from a repository. Refuses to delete the default branch.',
    inputSchema: {
      project: z.string().optional().describe('Project key. Defaults to BITBUCKET_DEFAULT_PROJECT.'),
      repository: z.string().describe('Repository slug.'),
    ...
    try {
      ...
      const defaultBranch = await clients.api.get(
      ...
      await clients.branchUtils.post(
