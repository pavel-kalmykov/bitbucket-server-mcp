...
export function registerSearchTools(
  ...
  server.registerTool('search', {
    description: 'Search for code or files across Bitbucket repositories. Supports filtering by project, repository, and search type.',
    inputSchema: {
      query: z.string().describe('Search query string.'),
      project: z.string().optional().describe('Project key to scope the search. Defaults to BITBUCKET_DEFAULT_PROJECT.'),
      repository: z.string().optional().describe('Repository slug to scope the search. Requires project to be set.'),
      type: z.enum(['code', 'file']).optional().describe('Search type: "code" for content search, "file" for filename search.'),
    ...
    try {
      ...
      const data = await clients.search.get('search', {
