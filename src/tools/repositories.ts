...
export function registerRepositoryTools(
  ...
  server.registerTool('list_projects', {
    description: 'List all Bitbucket projects you have access to. Use this first to discover project keys.',
    inputSchema: {
    ...
    try {
      const data = await clients.api.get('projects', {
  ...
  server.registerTool('list_repositories', {
    description: 'List repositories in a project. Use this to find repository slugs for other operations.',
    inputSchema: {
    ...
    try {
      ...
      const data = await clients.api.get(`projects/${resolvedProject}/repos`, {
        searchParams: { limit, start },
      }).json<{ values: unknown[]; size: number; isLastPage: boolean }>();
  ...
  server.registerTool('browse_repository', {
    ...
    inputSchema: {
      ...
      branch: z.string().optional().describe('Branch or commit hash (default: default branch).'),
  ...
  server.registerTool('get_file_content', {
    ...
    inputSchema: {
      ...
      branch: z.string().optional().describe('Branch or commit hash (default: default branch).'),
      limit: z.number().optional().describe('Max lines per request (default: 100, max: 1000).'),
      start: z.number().optional().describe('Starting line number (default: 0).'),
    },
    annotations: { readOnlyHint: true },
