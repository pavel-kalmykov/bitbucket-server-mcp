...
export function formatApiError(status: number, message: string): ToolErrorResult {
  let guidance: string;

  if (ERROR_GUIDANCE[status]) {
    guidance = ERROR_GUIDANCE[status];
  } else if (status >= 500) {
    guidance = 'Bitbucket server error. The server may be temporarily unavailable; try again.';
  } else {
...
export function isHttpError(error: unknown): error is { response: { status: number; data?: { message?: string } } } {
