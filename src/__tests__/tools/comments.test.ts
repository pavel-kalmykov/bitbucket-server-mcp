...
describe('Comment tools', () => {
  ...
  describe('manage_comment', () => {
    ...
    test('should create a draft comment with state PENDING', async () => {
      const mockResponse = { id: 2, text: 'Draft note', state: 'PENDING', version: 0 };
      ...
      expect(mockClients.api.post).toHaveBeenCalledWith(
        'projects/DEFAULT/repos/my-repo/pull-requests/42/comments',
        expect.objectContaining({
          json: expect.objectContaining({ text: 'Draft note', state: 'PENDING' }),
        }),
      );
    });

    test('should create an inline comment with filePath, line, and lineType', async () => {
      const mockResponse = { id: 3, text: 'Inline note', version: 0, anchor: { path: 'src/main.ts', line: 10, lineType: 'ADDED' } };
    ...
    test('should create a task comment with severity BLOCKER', async () => {
      const mockResponse = { id: 4, text: 'Fix this', severity: 'BLOCKER', version: 0 };
      ...
      expect(mockClients.api.post).toHaveBeenCalledWith(
        'projects/DEFAULT/repos/my-repo/pull-requests/42/comments',
        expect.objectContaining({
          json: expect.objectContaining({ text: 'Fix this', severity: 'BLOCKER' }),
        }),
      );
    });
  ...
  describe('submit_review', () => {
    test('should approve a pull request', async () => {
      const mockResponse = { approved: true, user: { name: 'admin' }, role: 'REVIEWER', status: 'APPROVED' };
    ...
    test('should publish a review with participantStatus APPROVED', async () => {
      const mockResponse = { user: { name: 'admin' }, role: 'REVIEWER', status: 'APPROVED' };
