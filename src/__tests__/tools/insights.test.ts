  ...
  describe('get_code_insights', () => {
    test('should fetch reports and annotations', async () => {
      ...
      const sonarAnnotations = {
        values: [
          { path: 'src/index.ts', line: 10, message: 'Bug found', severity: 'HIGH' },
        ],
      };

      const coverageAnnotations = {
        values: [
          { path: 'src/utils.ts', line: 5, message: 'Not covered', severity: 'LOW' },
        ],
      };

      (mockClients.insights.get as ReturnType<typeof vi.fn>).mockImplementation((url: string) => {
    ...
    test('should default annotations to empty array on error', async () => {
      const mockReports = {
        values: [
          { key: 'broken-report', title: 'Broken', result: 'PASS' },
        ],
      };
      ...
      (mockClients.insights.get as ReturnType<typeof vi.fn>).mockImplementation((url: string) => {
        ...
        return { json: () => Promise.reject(new Error('Annotations not available')) };
      });
