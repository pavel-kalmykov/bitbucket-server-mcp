...
describe('Search tools', () => {
  ...
  describe('search', () => {
    ...
    test('should prepend repo filter when repository is provided', async () => {
      ...
      expect(mockClients.search.get).toHaveBeenCalledWith('search', {
        searchParams: { query: 'repo:MYPROJ/my-repo TODO', limit: 25, start: 0 },
      });
    });
    ...
    test('should use default project for repo filter when project is not provided', async () => {
      ...
      expect(mockClients.search.get).toHaveBeenCalledWith('search', {
        searchParams: { query: 'repo:DEFAULT/my-repo TODO', limit: 25, start: 0 },
      });
    });
    ...
    test('should apply both repo filter and file type together', async () => {
      ...
      await client.callTool({
        name: 'search',
        arguments: { query: 'config.yaml', project: 'PROJ', repository: 'svc', type: 'file' },
      });

      expect(mockClients.search.get).toHaveBeenCalledWith('search', {
        searchParams: { query: '"repo:PROJ/svc config.yaml"', limit: 25, start: 0 },
      });
    });
