...
describe('Pull request tools', () => {
  ...
  describe('create_pull_request', () => {
    test('should create a basic pull request', async () => {
      ...
      (mockClients.defaultReviewers.get as ReturnType<typeof vi.fn>).mockReturnValueOnce({
  ...
  describe('update_pull_request', () => {
    ...
    test('should replace reviewers when provided', async () => {
      ...
      (mockClients.api.put as ReturnType<typeof vi.fn>).mockReturnValueOnce({
        json: () => Promise.resolve({ ...existingPr, reviewers: [{ user: { name: 'carol' } }] }),
      });
    ...
    test('should update target branch', async () => {
      ...
      (mockClients.api.put as ReturnType<typeof vi.fn>).mockReturnValueOnce({
        json: () => Promise.resolve({ ...existingPr, toRef: { id: 'refs/heads/develop', displayId: 'develop' } }),
      });
  ...
  describe('list_pull_requests', () => {
    test('should list pull requests with filters', async () => {
      const mockResponse = {
        values: [
          { id: 1, title: 'PR 1', author: { user: { name: 'alice', slug: 'alice', displayName: 'Alice' } } },
          { id: 2, title: 'PR 2', author: { user: { name: 'bob', slug: 'bob', displayName: 'Bob' } } },
        ],
        size: 2,
        isLastPage: true,
      };
  ...
  describe('get_pr_activity', () => {
    ...
    test('should filter to reviews only', async () => {
      ...
      const result = await client.callTool({
        name: 'get_pr_activity',
        arguments: { project: 'PROJ', repository: 'my-repo', prId: 1, filter: 'reviews' },
      });

      const content = result.content as Array<{ type: string; text: string }>;
      const parsed = JSON.parse(content[0].text);
      expect(parsed).toHaveLength(2);
      expect(parsed.every((a: { action: string }) => a.action === 'APPROVED' || a.action === 'REVIEWED')).toBe(true);
    });
    ...
    test('should filter to comments only', async () => {
      ...
      const result = await client.callTool({
        name: 'get_pr_activity',
        arguments: { project: 'PROJ', repository: 'my-repo', prId: 1, filter: 'comments' },
      });

      const content = result.content as Array<{ type: string; text: string }>;
      const parsed = JSON.parse(content[0].text);
      expect(parsed).toHaveLength(2);
      expect(parsed.every((a: { action: string }) => a.action === 'COMMENTED')).toBe(true);
    });
  });
  ...
  describe('get_diff', () => {
    ...
    test('should pass contextLines and withComments to searchParams', async () => {
      ...
      await client.callTool({
        name: 'get_diff',
        arguments: { project: 'PROJ', repository: 'my-repo', prId: 1, contextLines: 5 },
      });
    ...
    test('should not truncate when maxLinesPerFile is 0', async () => {
      ...
      const result = await client.callTool({
        name: 'get_diff',
        arguments: { project: 'PROJ', repository: 'my-repo', prId: 1, maxLinesPerFile: 0 },
      });
