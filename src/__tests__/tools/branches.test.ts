...
describe('Branch tools', () => {
  ...
  describe('list_branches', () => {
    test('should list branches and include default branch', async () => {
      ...
      const defaultBranchResponse = { displayId: 'main', id: 'refs/heads/main' };

      (mockClients.api.get as ReturnType<typeof vi.fn>).mockImplementation((url: string) => {
    ...
    test('should use default project when not provided', async () => {
      (mockClients.api.get as ReturnType<typeof vi.fn>).mockImplementation((url: string) => {
        ...
        return { json: () => Promise.resolve({ values: [], size: 0, isLastPage: true }) };
      });

      await client.callTool({ name: 'list_branches', arguments: { repository: 'my-repo' } });
  ...
  describe('list_commits', () => {
    test('should list commits for a branch', async () => {
      const mockResponse = {
        values: [
          { id: 'abc123', message: 'Initial commit', author: { name: 'john', displayName: 'John Doe', slug: 'jdoe' } },
          { id: 'def456', message: 'Second commit', author: { name: 'jane', displayName: 'Jane Smith', slug: 'jsmith' } },
        ],
        size: 2,
        isLastPage: true,
      };
    ...
    test('should filter commits by author (case-insensitive)', async () => {
      const mockResponse = {
        values: [
          { id: 'abc123', message: 'First', author: { name: 'john', displayName: 'John Doe', slug: 'jdoe' } },
          { id: 'def456', message: 'Second', author: { name: 'jane', displayName: 'Jane Smith', slug: 'jsmith' } },
        ],
        size: 2,
        isLastPage: true,
      };
    ...
    test('should use default project when not provided', async () => {
      ...
      await client.callTool({ name: 'list_commits', arguments: { repository: 'my-repo' } });
  ...
  describe('delete_branch', () => {
    test('should delete a non-default branch', async () => {
      (mockClients.api.get as ReturnType<typeof vi.fn>).mockReturnValue({
        json: () => Promise.resolve({ displayId: 'main', id: 'refs/heads/main' }),
      });

      (mockClients.branchUtils.post as ReturnType<typeof vi.fn>).mockReturnValue({
        json: () => Promise.resolve({}),
      });

      const result = await client.callTool({
        name: 'delete_branch',
        arguments: { project: 'TEST', repository: 'my-repo', branch: 'feature/old' },
      });
