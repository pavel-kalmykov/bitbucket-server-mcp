...
describe('Repository tools', () => {
  ...
  describe('list_projects', () => {
    test('should list projects with pagination', async () => {
      const mockResponse = {
        values: [
          { key: 'PROJ', name: 'Project', description: 'Test', public: false, type: 'NORMAL' },
        ],
        size: 1,
        isLastPage: true,
      };
  ...
  describe('list_repositories', () => {
    test('should list repositories for a project', async () => {
      const mockResponse = {
        values: [
          { slug: 'my-repo', name: 'My Repo', project: { key: 'TEST' }, state: 'AVAILABLE' },
        ],
        size: 1,
        isLastPage: true,
      };
  ...
  describe('get_file_content', () => {
    test('should read file content', async () => {
      ...
      const result = await client.callTool({
        ...
        arguments: { project: 'TEST', repository: 'my-repo', filePath: 'README.md' },
      });
