...
describe('truncateDiff', () => {
  ...
  test('should truncate file sections that exceed maxLines', () => {
    const lines = ['diff --git a/big.ts b/big.ts', 'index abc..def', '--- a/big.ts', '+++ b/big.ts', '@@ -1,50 +1,50 @@'];
    for (let i = 0; i < 50; i++) {
      lines.push(`+line ${i}`);
    }
  ...
  test('should preserve file headers even when truncating', () => {
    const lines = ['diff --git a/f.ts b/f.ts', 'index abc..def', '--- a/f.ts', '+++ b/f.ts', '@@ -1,20 +1,20 @@'];
  ...
  test('should handle multiple files', () => {
    const lines = [
      'diff --git a/a.ts b/a.ts', '--- a/a.ts', '+++ b/a.ts', '@@ -1,3 +1,3 @@', '+short',
      'diff --git a/b.ts b/b.ts', '--- a/b.ts', '+++ b/b.ts', '@@ -1,30 +1,30 @@',
    ];
